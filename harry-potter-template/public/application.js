...
var onChangePassword = function onChangePassword(event) {
  ...
  event.preventDefault();
  api.changePassword(data).then(ui.changePasswordSuccess).then($('#games-history').show()).catch(ui.changePasswordFailure);
};

var onSignOut = function onSignOut(event) {
  event.preventDefault();
  api.signOut().then(ui.signOutSuccess).then($('#board').hide()).then($('#change-password').hide()).then($('#sign-out').hide()).catch(ui.signOutFailure);
};

var onCreateGame = function onCreateGame(event) {
  // console.log('onCreateGame function works.')
  api.createGame().then(ui.createGameSuccess).then($('#games-history').show()).catch(ui.createGameFailure);
};

var onNewMove = function onNewMove(event) {
  event.preventDefault();
  // console.log('You have made a new move!')
  var gameOver = store.gameOver;
  // console.log(gameOver)
  var indexOfCell = $(event.target).attr('id');
  var player = $(event.target).text();
  // console.log(gameOver, indexOfCell, player)
  // console.log(store)
  api.newMove(gameOver, indexOfCell, player).then(ui.createGameSuccess).catch(ui.createGameFailure);
};

var onGameHistory = function onGameHistory(event) {
  event.preventDefault();
  // console.log('Game History Was pressed')
  api.gameHistory().then(ui.getGameHistorySuccess).catch(ui.getGameHistoryFailure);
};
...
var checkRow = function checkRow(a, b, c, turn) {
  var result = false;
...
var checkForWinner = function checkForWinner(turn) {
  // console.log('checkWinner()')
  // console.log(gameArray)
  // console.log(turn)
  var result = false;
  if (checkRow(0, 1, 2, turn) || checkRow(3, 4, 5, turn) || checkRow(6, 7, 8, turn) || checkRow(0, 3, 6, turn) || checkRow(1, 4, 7, turn) || checkRow(2, 5, 8, turn) || checkRow(0, 4, 8, turn) || checkRow(2, 4, 6, turn) || checkRow(0, 4, 8, turn)) {
    result = true;
    store.gameOver = true;
...
var newMove = function newMove(gameOver, indexOfCell, player) {
  return $.ajax({
...
var gameHistory = function gameHistory() {
  return $.ajax({
    url: config.apiOrigin + '/games',
    method: 'GET',
    headers: {
...
var signInFailure = function signInFailure() {
  // console.error(error)
  $('#message').text('Error with your login, buddy!').then($('#board').hide()).then($('#change-password').hide()).then($('#sign-out').hide());
};
...
var changePasswordFailure = function changePasswordFailure() {
  // console.error(error)
  $('#message').text('Error wtih changing your password, buddy!').then($('#board').hide()).then($('#change-password').hide()).then($('#sign-out').hide());
};
...
var signOutFailure = function signOutFailure() {
  // console.log(error)
  // console.error(error)
  $('#message').text('Error wtih signing out, buddy!').then($('#board').hide()).then($('#change-password').hide()).then($('#sign-out').hide());
};
...
var getGameHistorySuccess = function getGameHistorySuccess(data) {
...
var getGameHistoryFailure = function getGameHistoryFailure() {
  // console.error(error)
...
module.exports = {
  ...
  getGameHistorySuccess: getGameHistorySuccess,
  getGameHistoryFailure: getGameHistoryFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
